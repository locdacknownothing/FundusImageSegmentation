...
def prepare_data():
    ...
    if not os.path.exists(converted_dataset_path):
        ...
        task_tar = os.path.join(downloads_dir, "Task05_Prostate.tar")
        if not os.path.exists(task_tar):
            subprocess.run(
                ["wget", "-q", "-O", task_tar, "https://msd-for-monai.s3-us-west-2.amazonaws.com/Task05_Prostate.tar"],
                check=True
            )
        ...
        subprocess.run([
            ...
            "-i", os.path.join(downloads_dir, "Task05_Prostate"),
            "-overwrite_id", "005"
        ], check=True)
    else:
...
def find_best_configuration_and_postprocessing(fold: int, max_epochs: int):
    ...
    1. Determine the best postprocessing.
...
def main():
    fold = 4  # random.randint(0, 4) # ONE random fold
    max_epochs = 50
